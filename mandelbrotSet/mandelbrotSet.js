class Tile {
    ...
    static *tiles(width, height, numRows, numCols){
        ...
        for (let row = 0; row<numRows;row++){
            let titleHeight = (row < numRows-1) ? rowHeight : height - rowHeight * (numRows-1)
            for (let col = 0; col<numCols;col++){
                let titleWidth = (col < numCols-1) ? columnWidth : width - columnWidth * (numCols-1)
                yield new Tile(col * columnWidth, row * rowHeight, titleWidth, titleHeight)
            }
        }
    }
}
...
class PageState {
    ...
    static fromURL(url) {
        ...
        s.prePixel = parseFloat(u.searchParams.get("pp"))
...
class MandelbrotCanvas {
    ...
    render(){
        ...
        this.pendingRender = Promise.all(promises).then(responses => {
            ...
            for (let r of responses){
                this.contex.putImageData(r.imageData, r.tile.x, r.tile.y)
            }
        })
        .catch((reason) => {
    ...
    handlePointer(event){
        ...
        const pointerUpHandler = event => {
            ...
            if (dx > 10 || dy > 10 || dt > 500){
                this.state({cx: cx-dx*perPixel, cy: cy-dy*perPixel})
            }else{
